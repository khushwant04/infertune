...
```
KV working set  P50: 3.1 GiB   P95: 6.8 GiB   available: 5.4 GiB
→ preemption expected in ~9% of steady-state windows at target concurrency
→ mitigations, in order of cost: max_model_len 8192→4096, kv_cache_dtype fp8, TP 1→2
```
...
**M0 — Skeleton (0.5 wk).** Package layout, `core/` models, units handling (bytes vs GiB bugs are
endemic in this domain), CI on CPU.
*Accept:* `infertune --help` runs; core imports without torch installed.
...
Ledger (per GPU)
  ...
  weights (safetensors-measured, bf16)              16.06 GB   (14.96 GiB)
...
Workload: input ~1024 (p95 2048), output ~256 (p95 512)
  working set @ 24 concurrent   p50 ~27,600 tok = 3.37 GiB   p95 ~61,400 tok = 7.50 GiB
  BINDING CONSTRAINT: kv_working_set_p95 (7.50 GiB) > kv_budget (3.77 GiB)
  → fits at p50, preempts under p95 conditions
  → B* ≈ 51 for this GPU (82.6 TFLOPS bf16 w/ fp32 accum, 1008 GB/s, MFU 0.5 / MBU 0.8),
    so concurrency 24 is still bandwidth-bound — raising it is nearly free on latency,
    but there is no KV headroom to raise it. KV, not compute, is the wall.
  → mitigations, cheapest first:
      kv_cache_dtype=fp8   → 64 KiB/token → 61,800 tokens, just clears p95
      -O0 / enforce-eager  → reclaims the 1.2 GiB graph pool, costs decode perf
...
named, and that the mitigations are ordered by what they cost. That is the product.
