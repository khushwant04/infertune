...
Last updated: milestone M4 merged (PR #5).
...
| **M4** | SGLang adapter, engine registry, configuration search, `tune` | within **10%** of exhaustive grid using **≤12** boots | **0.00% gap** using **5** boots vs the grid's 30 | ✅ met |
...
**`tune --execute` has not run end to end.** The dry-run path (enumerate, prune, rank) is
exercised; the boot-and-sweep path is not.

## Remaining work

Four items, all needing one GPU session on a 2xA10 host:

1. Measure M3's ±20% throughput criterion against held-out configurations.
2. Run tp=2 over PCIe. Check early whether vGPU has disabled P2P — if it has, the measurement
   still stands but the latency term is being tested under a pessimistic interconnect.
3. Boot the SGLang adapter against a real install and diff its introspected schema against the
   fixture.
4. `tune --execute` end to end, and confirm the executed winner matches the dry-run ranking.
...
closed the gap to 0.00%, so there is no evidence it would buy anything.
